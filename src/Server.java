...
public abstract class Server
{
    ...
    protected class ClientMessageHandler extends Thread
    {
        ...
        public void sendEncrypted(String pMessage, Key aesKey, EncryptionLib el) throws Exception {
            String ciphertext = "ENCRYPTED " +
                    el.AESencrypt(pMessage, aesKey) + " " +
                    EncryptionLib.calculateHMAC(el.AESencrypt(pMessage, aesKey),
                            Base64.getEncoder().encodeToString(aesKey.getEncoded()) +
                                    // OTP Generator
                                    Math.floor((Instant.now().getEpochSecond() - 1) / 30)
                    );

            if(active)
                socketWrapper.send(ciphertext);
        }
    ...
    public void sendEncrypted(String pClientIP, int pClientPort, String pMessage, Key aesKey, EncryptionLib el) throws Exception {
        ClientMessageHandler aMessageHandler = this.findClientMessageHandler(pClientIP, pClientPort);
        if (aMessageHandler != null)
            aMessageHandler.sendEncrypted(pMessage, aesKey, el);
    }


    public abstract void sendToAll(String pMessage) throws Exception;
    ...
    public abstract void processMessage(String pClientIP, int pClientPort, String pMessage) throws Exception;
